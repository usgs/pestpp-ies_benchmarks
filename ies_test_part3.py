...
def clues_longnames_test():
    ...
    pst.control_data.noptmax = 1
    pst.write(os.path.join(template_d, "pest.pst"))
    pyemu.os_utils.start_slaves(template_d, exe_path.replace("-ies", "-glm"), "pest.pst", 5,
                                slave_root=model_d, master_dir=test_d,port=port)
...
def tenpar_tied_test():
    ...
    pst.write(pst_name)
    pyemu.os_utils.start_slaves(template_d, exe_path, "pest_tied.pst", num_slaves=5,
...
def tenpar_by_vars_test():
    ...
    pst.write(pst_name)
    pyemu.os_utils.start_slaves(template_d, exe_path, "pest_vars.pst", num_slaves=10,
                                master_dir=test_d, verbose=True, slave_root=model_d,
                                port=port)
